...
export class ForumsCollector implements Collector {
  id = 'forums'
  ...
  async *fetch(opts: FetchOptions, signal?: AbortSignal): AsyncIterable<RawReview> {
    ...
    try {
      ...
      if (!fs.existsSync(csvPath)) {
        console.error(`[ForumsCollector] Seed corpus not found at: ${csvPath}`)
        return
      }
      ...
      console.error(`[ForumsCollector] Error: ${errMsg}`)
    }
  }
}
